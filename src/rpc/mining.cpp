...
static UniValue generatetoaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"generatetoaddress",
                "\nMine blocks immediately to a specified address (before the RPC call returns)\n",
                {
                    ...
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated litecoinz to."},
            ...
            + "If you are running the litecoinz core wallet, you can get a new address to send the newly generated litecoinz to with:\n"
...
// NOTE: Unlike wallet RPC (which use LTZ values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0 && Params().NetworkIDString() != CBaseChainParams::REGTEST)
    ...
    if (pindexPrev != ::ChainActive().Tip() ||
        ...
        fLastTemplateSupportsSegwit != isSegwitEnabled)
    {
        ...
        pblocktemplate = BlockAssembler(Params()).CreateNewBlock(scriptDummy, isSegwitEnabled);
        if (!pblocktemplate)
