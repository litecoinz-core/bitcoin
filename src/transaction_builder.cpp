...
TransactionBuilderResult::TransactionBuilderResult(const CTransactionRef& tx) : maybeTx(tx) {}
...
{
    if (provider == nullptr) {
        throw std::runtime_error("Cannot add transparent inputs to a TransactionBuilder without a provider");
    }
...
TransactionBuilderResult TransactionBuilder::Build()
{
    ...
    CScript scriptCode;
    try {
        dataToBeSigned = SignatureHash(scriptCode, mtx, NOT_AN_INPUT, SIGHASH_ALL, 0, sigversion, consensusBranchId);
    ...
    for (size_t nIn = 0; nIn < mtx.vin.size(); nIn++) {
        ...
        bool signSuccess = ProduceSignature(*provider,
            MutableTransactionSignatureCreator(&mtx, nIn, tIn.value, SIGHASH_ALL), tIn.scriptPubKey, sigdata, consensusBranchId);
    ...
    return TransactionBuilderResult(MakeTransactionRef(std::move((mtx))));
}
...
void TransactionBuilder::CreateJSDescriptions()
{
    ...
    size_t changeOutputIndex = -1; // this is updated after each joinsplit if jsChange > 0
    ...
    while (!vpubNewProcessed) {
        ...
        if (jsChange > 0) {
            ...
            auto hSig = ZCJoinSplit::h_sig(prevJoinSplit.randomSeed, prevJoinSplit.nullifiers, mtx.joinSplitPubKey);
            try {
