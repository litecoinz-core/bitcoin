...
AsyncRPCOperation_mergetoaddress::AsyncRPCOperation_mergetoaddress(
    ...
    request_(request), tx_(MakeTransactionRef(std::move(contextualTx))), utxoInputs_(utxoInputs), sproutNoteInputs_(sproutNoteInputs),
...
bool AsyncRPCOperation_mergetoaddress::main_impl()
{
    ...
    if (isUsingBuilder_) {
        ...
        std::vector<libzcash::SaplingNote> saplingNotes;
        std::vector<libzcash::SaplingExpandedSpendingKey> expsks;
    ...
    if (sproutNoteInputs_.empty() && isToZaddr_) {
        ...
        libzcash::JSOutput jso = libzcash::JSOutput(boost::get<libzcash::SproutPaymentAddress>(toPaymentAddress_), sendAmount);
        if (hexMemo.size() > 0) {
    ...
    libzcash::SproutSpendingKey changeKey = std::get<3>(zInputsDeque.front());
    libzcash::SproutPaymentAddress changeAddress = changeKey.address();
    ...
    while (!vpubNewProcessed) {
        ...
        if (jsChange > 0) {
            ...
            if (it != intermediates.end()) {
                tree = it->second;
            } else if (!::ChainstateActive().CoinsTip().GetSproutAnchorAt(prevJoinSplit.anchor, tree)) {
            ...
            try {
                ...
                libzcash::SproutNote note = plaintext.note(changeAddress);
        ...
        while (numInputsNeeded++ < ZC_NUM_JS_INPUTS && zInputsDeque.size() > 0) {
            ...
            int wtxDepth = -1;
            {
                ...
                if (!pindex) {
                    throw JSONRPCError(RPC_WALLET_ERROR, strprintf("mapBlockIndex does not contain block hash %s", wtx.m_confirm.hashBlock.ToString()));
                }
        ...
        // create dummy output
        info.vjsout.push_back(libzcash::JSOutput()); // dummy output while we accumulate funds into a change note for vpub_new

        // create output for any change
        if (jsChange > 0) {
            std::string outputType = "change";
            auto jso = libzcash::JSOutput(changeAddress, jsChange);
    ...
    uint256 anchor)
{
    ...
    for (size_t i = 0; i < witnesses.size(); i++) {
        ...
        info.vjsin.push_back(libzcash::JSInput(*witnesses[i], info.notes[i], info.zkeys[i]));
    }
