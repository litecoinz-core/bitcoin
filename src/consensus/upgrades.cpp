...
uint32_t CurrentEpoch(int nHeight, const Consensus::Params& params) {
    for (uint32_t idxInt = Consensus::MAX_NETWORK_UPGRADES - 1; idxInt >= Consensus::BASE_SPROUT; idxInt--) {
...
bool IsConsensusBranchId(uint32_t branchId) {
    for (uint32_t idx = Consensus::BASE_SPROUT; idx < Consensus::MAX_NETWORK_UPGRADES; idx++) {
...
boost::optional<uint32_t> NextEpoch(int nHeight, const Consensus::Params& params) {
    if (nHeight < 0) {
        return boost::none;
    }
...
boost::optional<uint32_t> NextActivationHeight(
    int nHeight,
    const Consensus::Params& params)
{
