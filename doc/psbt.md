...
PSBT is an interchange format for Litecoinz transactions that are not fully signed
...
Overall, the construction of a fully signed Litecoinz transaction goes through the
following steps:
...
- An **Extractor** produces a valid Litecoinz transaction (in network format)
...
#### Multisig with multiple Litecoinz Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Litecoinz Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `litecoinz-cli` is used.
